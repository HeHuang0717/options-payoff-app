...
# =========================
def simulate(initial_capital: float,
             odds: float,
             win_prob: float,
             bet_fraction: float,
             n_bets: int,
             ...
             seed: int):
    """
    Fixed-stake Monte Carlo simulation.
    - Bet amount = bet_fraction * initial_capital (fixed each round)
    ...
    bet_amount = initial_capital * bet_fraction
    ...
    for t in range(1, n_bets + 1):
        ...
        # Apply only to still-alive paths
        win_mask = alive & outcomes
        lose_mask = alive & (~outcomes)

        capital[win_mask] += bet_amount * (odds - 1.0)
        capital[lose_mask] -= bet_amount

        # clip to 0 and update alive
...
# Plot: all paths (fast)
# =========================
def plot_all_paths(all_paths, initial_capital=100, title="All Equity Paths"):
    ...
    plt.title("Sample Equity Curves (10 out of 1000 simulations, with stop at bankruptcy)")
...
def plot_equity_heatmap(all_paths, n_bins=200, normalize="column", log_scale=False, initial_capital=100):
    ys = all_paths
    H, (ymin, ymax) = _heatmap_matrix(ys, n_bins=n_bins, normalize=normalize, log_scale=log_scale)
    T = ys.shape[1]
    x = np.arange(T)

    H_plot = np.ma.masked_where(H == 0, H)  # 屏蔽所有 0
    cmap = mpl.cm.rainbow.copy()
    cmap.set_bad(color='white')

    fig, ax = plt.subplots(figsize=(10, 6), dpi=120)
    im = ax.imshow(H_plot, origin="lower", aspect="auto",
                   extent=(0, T - 1, ymin, ymax), cmap="rainbow")
    ...
    # quantiles
    for q, lw in zip((10, 50, 90), (2.2, 2.8, 2.2)):
        ax.plot(x, np.percentile(ys, q, axis=0), linewidth=lw, label=f"{q}th pct")

    ax.axhline(initial_capital, linestyle="-", linewidth=2, alpha=1, label="Initial capital",color='k')
    ax.set_title("Equity Distribution Heatmap")
...
def plot_final_hist(final_capitals):
    fig, ax = plt.subplots(figsize=(10, 6), dpi=120)
    ax.hist(final_capitals, bins=30, edgecolor="black", alpha=0.75)
...
initial_capital = st.sidebar.number_input("Initial capital", min_value=1.0, value=100.0, step=10.0)
...
bet_fraction = st.sidebar.number_input("Bet fraction of initial capital", min_value=0.0, max_value=1.0, value=0.1, step=0.01)
n_bets = st.sidebar.number_input("Number of bets per simulation", min_value=1, value=500, step=50)
n_sims = st.sidebar.number_input("Number of simulations", min_value=1, value=1000, step=500)
...
st.sidebar.markdown("---")
max_lines = st.sidebar.slider("Max paths drawn in line plot", min_value=50, max_value=5000, value=100, step=50)
...
st.caption("Capital never goes below 0. Once it hits 0, the path stays at 0 (stop betting). "
           "Bet size = bet_fraction × initial_capital (fixed each round).")
...
fig_paths = plot_all_paths(all_paths[:paths_to_draw], initial_capital=initial_capital,
                           title=f"Equity Paths (showing {paths_to_draw} of {all_paths.shape[0]})")
...
with c2:
    fig_hist = plot_final_hist(final_capitals)
